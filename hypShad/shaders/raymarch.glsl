...
BEGIN FRAGMENT
  ...
  float globalSceneSDF(vec4 samplePoint, mat4 globalTransMatrix){
    ...
    for(int i=0; i<1; i++){
      float objDist;
      objDist = sphereSDF(absoluteSamplePoint, lightPositions[i] * globalTransMatrix, 1.0/(10.0*lightIntensities[i].w));
    ...
    float objDist;
    objDist = sphereSDF(absoluteSamplePoint, globalObjectBoost[3] * globalTransMatrix, globalObjectRadius);
    distance = min(distance, objDist);
    if(distance < EPSILON){
      hitWhich = 2;
    }
  ...
  /*float shadowMarch(vec4 origin, vec4 dirToLight, float distToLight, mat4 totalFixMatrix){
    float globalDepth = 0.1;
    for(int i = 0; i< MAX_MARCHING_STEPS; i++){
      vec4 globalEndPoint = pointOnGeodesic(origin, dirToLight, globalDepth);
      float globalDist = globalSceneSDF(globalEndPoint,  totalFixMatrix);
      if(globalDist < EPSILON && hitWhich != 1){
        return 0.25;
      }
      globalDepth += globalDist;
      if(globalDepth > distToLight || hitWhich == 1){
        return 1.0;
      }
    }
    return 1.0;
  ...
  float shadowMarch(vec4 origin, mat4 totalFixMatrix){
    float globalDepth = 0.1;
    vec4 lP = lightPositions[0] * invCellBoost * totalFixMatrix;
    vec4 dirToLight = hypDirection(origin, lP);
    float distToLight = hypDistance(origin, lP);
    for(int i = 0; i< MAX_MARCHING_STEPS; i++){
      vec4 globalEndPoint = pointOnGeodesic(origin, dirToLight, globalDepth);
      float globalDist = globalSceneSDF(globalEndPoint, totalFixMatrix);
      if(globalDist < EPSILON && hitWhich != 1){
  ...
  vec3 lightingCalculations(vec4 SP, vec4 TLP, vec4 V, vec3 baseColor, vec4 lightIntensity, mat4 totalFixMatrix){
    ...
    //shadow = shadowMarch(SP, L, distToLight, totalFixMatrix);
    shadow = shadowMarch(SP, totalFixMatrix);
  ...
  vec3 phongModel(mat4 totalFixMatrix){
    ...
    for(int i = 0; i<1; i++){ 
        ...
        color += lightingCalculations(SP, TLP, V, vec3(1.0), lightIntensities[i], totalFixMatrix);
    }
    return color;
  }
  ...
  vec4 estimateNormal(vec4 p) { // normal vector is in tangent hyperplane to hyperboloid at p
      ...
      if(hitWhich != 3){ //global light scene
        return hypNormalize( //p+EPSILON*basis_x should be lorentz normalized however it is close enough to be good enough
          basis_x * (globalSceneSDF(p + newEp*basis_x, mat4(1.0)) - globalSceneSDF(p - newEp*basis_x, mat4(1.0))) +
          basis_y * (globalSceneSDF(p + newEp*basis_y, mat4(1.0)) - globalSceneSDF(p - newEp*basis_y, mat4(1.0))) +
          basis_z * (globalSceneSDF(p + newEp*basis_z, mat4(1.0)) - globalSceneSDF(p - newEp*basis_z, mat4(1.0))));
      }
      else{ //local scene
        return hypNormalize(
  ...
  void raymarch(vec4 rO, vec4 rD, out mat4 totalFixMatrix){
    ...
    for(int i = 0; i < MAX_MARCHING_STEPS; i++){
      ...
      float globalDist = globalSceneSDF(globalEndPoint, mat4(1.0));
